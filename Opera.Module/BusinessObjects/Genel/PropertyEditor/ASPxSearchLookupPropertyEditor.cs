...
{
    [PropertyEditor(typeof(XPLiteObject), false)]//EditorAliases.LookupPropertyEditor, true)]
    public class ASPxSearchLookupPropertyEditor : ASPxObjectPropertyEditorBase, IDependentPropertyEditor, ITestable, ISupportViewShowing, IFrameContainer
    {
        ...
        private PopupWindowShowAction findObjectWindowAction;
        ...
        private void UpdateDropDownLookup(WebControl editor) {
            ...
            if(supportNewObjectCreating != null) {
                ...
                if(application != null) {
                    ...
                    supportNewObjectCreating.SetClientFindButtonScript(application.PopupWindowManager.GenerateModalOpeningScript(editor, findObjectWindowAction, windowWidth, windowHeight, false, supportNewObjectCreating.GetProcessFindObjFunction()));
                }                
            }
        }
        ...
        private void newObjectViewController_ObjectCreating(object sender, ObjectCreatingEventArgs e) {
            ...
            // B196715
#warning 1
            //if(e.ObjectSpace is NestedObjectSpace) {
        ...
        private void findObjectWindowAction_OnCustomizePopupWindowParams(Object sender, CustomizePopupWindowParamsEventArgs args)
        {
            if (!this.DataSource.AllowAdd)
            {
                throw new InvalidOperationException();
            ...
            newObjectViewController.NewObjectAction.DoExecute(newObjectViewController.NewObjectAction.Items[0]);
            args.View = application.CreateListView(objectSpace, MemberInfo.MemberType, false);
        }
        private void findObjectWindowAction_OnExecute(Object sender, PopupWindowShowActionExecuteEventArgs args)
        {
            if (!this.DataSource.AllowAdd)
            {
                throw new InvalidOperationException();
            ...
            ListView _listView = (ListView)args.PopupWindow.View;
            object val = helper.ObjectSpace.GetObject(_listView.CurrentObject);
            DataSource.Add(val);
            ((PopupWindow)args.PopupWindow).ClosureScript = "if(window.opener != null) window.opener.ddLookupResult = '" + _listView.ObjectSpace.GetKeyValueAsString(val) + "';";

            //DetailView dv = View as DetailView;
            //foreach (PropertyEditor editor in dv.GetItems<PropertyEditor>())
            //{
            //    if (editor.PropertyName == MemberInfo.Name)
            //    {
            //        ASPxSearchLookupPropertyEditor control = editor as ASPxSearchLookupPropertyEditor;
            //        if (control != null)
            //        {
            ...
            //if (CurrentObject != null && val != null)
            //{
            //    string keyVal = helper.GetDisplayText(val, EmptyValue, DisplayFormat);
            //    OnValueStoring(keyVal);
            //    MemberInfo.SetValue(CurrentObject, val);
            //    MemberInfo.SetValue(frame.ViewItem.CurrentObject, val);

            
            //}

            //((PopupWindow)args.PopupWindow).ClosureScript = "if(window.opener != null) window.opener.ddLookupResult = '" + _listView.ObjectSpace.GetKeyValueAsString(_listView.CurrentObject) + "';";
        }
        ...
        protected override WebControl CreateEditModeControlCore() {
            if(newObjectWindowAction == null) {
                newObjectWindowAction = new PopupWindowShowAction(null, "Yeni", PredefinedCategory.Unspecified.ToString());
            ...
            if (findObjectWindowAction == null) {
                findObjectWindowAction = new PopupWindowShowAction(null, "Bul", PredefinedCategory.Unspecified.ToString());
                findObjectWindowAction.Execute += new PopupWindowShowActionExecuteEventHandler(findObjectWindowAction_OnExecute);
                findObjectWindowAction.CustomizePopupWindowParams += new CustomizePopupWindowParamsEventHandler(findObjectWindowAction_OnCustomizePopupWindowParams);
                findObjectWindowAction.Application = helper.Application;
            }
    ...
    public class ASPxSearchDropDownEdit : Table, INamingContainer, ICallbackEventHandler {
        ...
        private string findButtonScript;
        ...
        private void UpdateClientButtonsScript() {
            dropDown.ClientSideEvents.ButtonClick = @"function(s, e) {
                ...
                @"}
                  if(e.buttonIndex == 1) {" +
                    findButtonScript + //findButtonScript
                @"}
                if(e.buttonIndex == 2) {" +
                    clearButtonScript +
                @"}             
        ...
        public ASPxSearchDropDownEdit(WebLookupEditorHelper helper, string emptyValue, string displayFormat) {
            ...
            dropDown.ItemRequestedByValue += new ListEditItemRequestedByValueEventHandler(dropDown_ItemRequestedByValue);
            dropDown.ItemsRequestedByFilterCondition += new ListEditItemsRequestedByFilterConditionEventHandler(dropDown_ItemsRequestedByFilterCondition);
            ...
            //TODO Add columns
            if(Helper.LookupObjectTypeInfo.Type.FullName == "Mikrobar.Module.BusinessObjects.Contact") {
                dropDown.Columns.Add("FullName", "FullName", 300);
                dropDown.Columns.Add("SpouseName", "SpouseName", 300);
                dropDown.TextFormatString = "{0} {1}";
            }

            if (Helper.LookupObjectTypeInfo.Type.FullName == "Mikrobar.Module.BusinessObjects.Istasyonlar")
            {
                dropDown.Columns.Add("IstasyonKod", "IstasyonKod", 300);
                dropDown.Columns.Add("IstasyonAd", "IstasyonAd", 300);
                //dropDown.TextFormatString = "{0}";
            }

            newButton = dropDown.Buttons.Add();
            findButton = dropDown.Buttons.Add();
            clearButton = dropDown.Buttons.Add();
            ASPxImageHelper.SetImageProperties(newButton.Image, "Action_New_12x12");
            ASPxImageHelper.SetImageProperties(findButton.Image, "Action_Search");
        ...
        public void SetClientFindButtonScript(string value)
        {
            findButtonScript = value;
            UpdateClientButtonsScript();
        }
        ...
        public string FindActionCaption
        {
            get
            {
            ...
            set
            {
                findButton.ToolTip = value;
                if (findButton.Image.IsEmpty)
                {
                    findButton.Text = value;
                }
            }
        }
        ...
        public string GetProcessFindObjFunction()
        {
            return "xafDropDownLookupProcessFindObject('" + UniqueID + "')";
        }
