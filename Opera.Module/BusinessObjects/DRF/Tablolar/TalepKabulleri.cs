...
namespace Mikrobar.Module.BusinessObjects
{
    ...
    XafDisplayName("Malzeme Talep Kabul Belgeleri"), XafDefaultProperty("TalepNo")]
    [DefaultClassOptions, OptimisticLocking(false), DeferredDeletion(false)]
    public class TalepKabulleri : XPObject // UPDATE dbo.Talepler SET Durum = 5, Aciklama3 = N'Talep süresi doldu kapatıldı.', GuncellemeTarihi = GETDATE() WHERE CONVERT(DATE, GecerlilikSuresi) < CONVERT(DATE, GETDATE())
    {                
        ...
        [ImmediatePostData]
        [Appearance("Talepturudegistirilemez1.", Enabled = false, Criteria = "Oid > 0", AppearanceItemType = "ViewItem", Context = "DetailView", TargetItems = "TalepTur")]
        ...
        [XmlIgnore(), Association("TalepKabulleri.Talepler.Talep"), Description("Talep master bilgisi")]
        ...
        #region Stok Hareket
        [PersistentAlias("Iif(StokHareket is null, 0, StokHareket.Oid)"), VisibleInListView(false), VisibleInLookupListView(false), VisibleInDetailView(false)]
        public int StokHareketId
        {
            get
            {
                return Convert.ToInt32(EvaluateAlias("StokHareketId"));
            }
            set
            {
                if (!IsLoading && !IsSaving)
                    SetPropertyValue<StokHareketleri>("StokHareket", ref _stokHareket, this.Session.GetObjectByKey<StokHareketleri>(value));
            }
        }

        private StokHareketleri _stokHareket;
        [XmlIgnore(), Association("TalepKabulleri.StokHareketleri.StokHareket"), NoForeignKey,
        Persistent("StokHareketId"), Description("Talep master bilgisi")]
        public StokHareketleri StokHareket
        {
            get { return _stokHareket; }
            set { SetPropertyValue<StokHareketleri>("StokHareket", ref _stokHareket, value); }
        }
        #endregion

        #region Kabul Stok Hareket
        [PersistentAlias("Iif(KabulStokHareket is null, 0, KabulStokHareket.Oid)"), VisibleInListView(false), VisibleInLookupListView(false), VisibleInDetailView(false)]
        public int KabulStokHareketId
        {
            get
            {
                return Convert.ToInt32(EvaluateAlias("KabulStokHareketId"));
            }
            set
            {
                if (!IsLoading && !IsSaving)
                    SetPropertyValue<StokHareketleri>("StokHareket", ref _kabulStokHareket, this.Session.GetObjectByKey<StokHareketleri>(value));
            }
        }

        private StokHareketleri _kabulStokHareket;
        [XmlIgnore(), Association("TalepKabulleri.StokHareketleri.KabulStokHareket"), NoForeignKey,
        Persistent("KabulStokHareketId"), Description("Talep master bilgisi")]
        public StokHareketleri KabulStokHareket
        {
            get { return _kabulStokHareket; }
            set { SetPropertyValue<StokHareketleri>("KabulStokHareket", ref _kabulStokHareket, value); }
        }
        #endregion
        ...
        [XmlIgnore(), Association(@"TalepKabulleri.TalepDepo.TalepDepoId"), NoForeignKey, Persistent("TalepDepoId"), XafDisplayName("Talep Depo"), ImmediatePostData]
        ...
        [XmlIgnore(), Association(@"TalepKabulleri.TalepEdilenDepo.TalepEdilenDepoId"), NoForeignKey, Persistent("TalepEdilenDepoId"), XafDisplayName("Talep Edilen Depo"), ImmediatePostData]
        ...
        [XmlIgnore(), Association(@"TalepKabulleri.Kullanici.KullaniciId"), NoForeignKey, Persistent("KullaniciId"), XafDisplayName("Gorevli Kullanici"), ImmediatePostData]
        ...
        [Association("TalepKabulDetaylari_TalepKabul"), XmlIgnore()]
        public XPCollection<TalepKabulDetaylari> Malzemeler
        {
            get { return GetCollection<TalepKabulDetaylari>("Malzemeler"); }
        }
...
ICity City { get; set; }
        ...
        public TalepKabulleri() { }
        public TalepKabulleri(Session session) : base(session) { }
    }
}
